...
#define NO_CONN_ID 0xffff
  ...
  //FB添加		#include "/opt/new/yate_fun_unit.cpp"

#include "../FB_fun_unit.cpp"

  YBTSDriver * g_Driver=0;
  ...
  public:
    ...
    //FB添加
    void NewLAI(char * mcc,char * mnc,char * lac)
        {
            this->m_lai.set(mcc, mnc,lac);
            //m_lai << m_mcc_mnc << "_" << m_lac;        
        }
    //FB添加完成
  ...
  public:
    ...
    virtual bool msgExecute_Uplink(Message& msg, String& dest);
  ...
  protected:
    ...
    void stop();
    bool startPeer();
    //FB 移    bool handleMsgExecute(Message& msg, const String& dest);
    ...
    //FB 移  virtual bool msgExecute(Message& msg, String& dest);
...
#undef MAKE_SIG
  ...
  static int decodeRP(const String& str, uint8_t& rpMsgType, uint8_t& rpMsgRef,
                      String* bcd = 0, String* plan = 0, String* type = 0, String* smsParty =
                      0, String* smsPartyPlan = 0, String* smsPartyType = 0,
                      String* smsText = 0, String* smsTextEnc = 0)
  {
    FB_DEBUG;
    //str=000d000891683108501705f01531420781489135f20000ff0ad8f1786c1e8fc7e331,rpMsgType=0,rpMsgRef=0(null)
    //000f000891683108501705f01635430781489135f20008ff0a4e2d4e2d4e2d4e2d4e2d		中中中中中
    //printf("===========str=%s,rpMsgType=%d,rpMsgRef=%d",str.c_str(),rpMsgType,rpMsgRef);
    //if (smsText)
    //printf("smsText=%s",smsText->c_str());
    //	printf("\n");
    DataBlock d;
    if (!d.unHexify(str))
    {
    ...
    if (tpMTI == 1)
    {
      ...
      //原文件 注释掉，改个返回值，方便面后的代码知道数据类型，交给APP单独处理
      //
      //		if (*b)		//编码类型
      //			return 0;
      ...
      if (vp)			//ff0ad8f1786c1e8fc7e33100			ff是要跳过的，跳过长度，看前面定义
      {
        l = (vp == 2) ? 1 : 7;
        if (l > len)
          l = len;
      ...
      l = *b++;			//数据长度		0ad8f1786c1e8fc7e33100		L=0x0A
      len--;
    ...
    if (conn)
    {
      ...
      if (conn)
      {
        ...
        if (ue)
        {
          ...
          SendData_ConnRelease(imei, imsi, tmsi);
...
#define YBTS_LOCUPD_CHECK_STOP { \
  ...
  void KickOut( YBTSUE* ue, YBTSConn* conn, const char * imei, const char * imsi, const char * tmsi, uint8_t cause=13)
  {
      /*
    {"PLMN-not-allowed",                                    0x0b},
    {"location-area-not-allowed",                           0x0c},
    {"roaming-not-allowed-in-this-location-area",           0x0d},
       * cause          可能再也不会入网，需要手工选择
       * cause=12   可能再也不会入网，需要手工选择
       * cause=13 可以正常切换到其它bts上，但是会有几秒的无信号（也可能我信号不好的原因）      
       * 但按理，好像应该是 12, 13会重新plmn选择
       * 
       * 11-13 手机会清除lAI,TMSI,密钥,并重新计数器,并把LAI,PLMNID加入禁止列表中
       * 并采用系统保留的LAI来重新入网
       * 11:
       *  PLMN加入禁止列表
       * 12:
      ...
      */
    YBTSMessage m;    
    m.setConnId (conn->connId ());
    g_Driver->mm()->sendLocationUpdateReject(m, conn, cause);

    NamedList params=NamedList("");
    params.addParam ("askimsi", "0");
    params.addParam ("askimei", "0");
    //if (tmsi)
    //params.addParam("tmsi",tmsi);
    if (imsi)
      params.addParam("imsi", imsi);
    if (cause)
    {
      params.addParam(s_error, String(cause));
      params.addParam(s_reason, String(cause));
      //params.addParam(s_reason,String("roaming-not-allowed-in-this-location-area"));
      //params.addParam(s_error,String("roaming-not-allowed-in-this-location-area"));
    }
    __plugin.mm()->locUpdTerminated(0, ue, conn->connId (), false, params);
  ...
  void YBTSLocationUpd::run()
  {
    ...
    //FB添加/修改		 YBTSLocationUpd::run()

    int r=SendData_ConnRequest(m_ue->imei().c_str(), m_ue->imsi().c_str(), 0);
    //		printf("==========验证========SendData_ConnRequest=%d\n",r);
#ifdef AppConntErr_AllowNoLogin
    if (r == -1)
      r=0;
#endif
    /*
     http://www.njliaohua.com/lhd_3evw21kf860fvqu4zj4f_1.html
     */
    if (r == 0)
    {
      KickOut(m_ue, this->m_conn, m_ue->imei().safe (), m_ue->imsi().safe (), "");
      //notify(false, true);
      return;
    }
      ...
      //if (ok)
    {
      //FB添加
      FB_DEBUG;
      SendData_Event(mt_Register, m_ue->imei().c_str(), m_ue->imsi().c_str(), m_ue->tmsi().c_str());
      //FB添加完成
  ...
  void YBTSLocationUpd::notify(bool final, bool ok)
  {
    ...
    else
    {
      ...
      //printf("----------xxxxx------------\n");
      m_msg.setParam(s_error, String(11));
      if (!Engine::exiting())
        Alarm(&__plugin, "system", DebugWarn,
    ...
    if (ok)
    {
      ...
      if (tmsi)
      {
        ...
        if (tmsi != ue->tmsi() || imsi != ue->imsi())
        {
          ...
          //FB添加 //2015-9-21
          FB_DEBUG;
          SendData_Event(mt_Register, ue->imei().c_str(), ue->imsi().c_str(), ue->tmsi().c_str());
          //FB添加完成
        }
      } else
      {
  ...
  Message* YBTSMM::buildUnregister(YBTSUE* ue, const String& ident, bool tmsi)
  {
    ...
    //FB添加
    SendData_Event(mt_UnRegister, ue->imei().c_str() , ue->imsi().c_str(),	ue->tmsi().c_str());
    //FB
  ...
  void YBTSMM::handleLocationUpdate(YBTSMessage& m, const XmlElement* xml, YBTSConn* conn)
  {
    ...
    ue->unlock();
    //printf("-------------FB--------sendLocationUpdateReject(m,conn,CauseProtoError);\n");
  ...
  void YBTSMM::handleIMSIDetach(YBTSMessage& m, const XmlElement& xml, YBTSConn* conn)
  {
    ...
    //FB添加
    if (ident)
      SendData_Event(mt_UnRegister, 0, 0, ident->c_str());
    //FB
    ...
    SendData_Event(mt_UnRegister, imei, imsi, tmsi);
    FB_DEBUG;
    //SendData_Event(mt_UnRegister,ue->imei().c_str() ,ue->imsi().c_str(),	ue->tmsi().c_str());
    //FB添加完成
  ...
  // Init outgoing chan. Return false to destruct the channel
  //初始化传出通道。返回false就销毁通道,必须号码存在的

  bool YBTSChan::initOutgoing(Message& msg)
  {
    /*
    printf("==============\n");
      printf("===========caller=%s\n",msg.getValue(YSTRING("caller")));
        printf("==========callernumplan=%s\n",msg.getValue(YSTRING("callernumplan")));
        printf("=========callernumtype=%s\n",msg.getValue(YSTRING("callernumtype")));
        printf("=========called=%s\n",msg.getValue(YSTRING("called")));
     */
    if (m_pending || !ue())
      return false;
    Lock lck(m_mutex);
  ...
  bool YBTSChan::initMT()
  {
    ...
    //FB_DEBUG_I("m_conn",m_conn);
    //FB_DEBUG_I("s_authMtCall",s_authMtCall);
                                   ...
                                   const String& callRef, bool tiFlag, const XmlElement& cpData)
  {
    if (!conn)		//早就发出了，
    {
      Debug(this, DebugMild, "Ignoring SMS CP-DATA conn=%u: no connection",
            m.connId());
      return;
    }
    FB_DEBUG;
    //	printf("=========%d===callref=%s,%d,cpdata=\n%s m=%s\n",__LINE__,callRef.c_str(),tiFlag,cpData.getText().c_str(),m.toString().c_str());
...
#define SMS_CPDATA_DONE_MILD(str) { level = DebugMild; SMS_CPDATA_DONE(str); }
      ...
      int res = decodeRP(*rpdu, rpMsgType, rpMsgRef, &called, &plan, &type,	&smsCalled, &smsCalledPlan, &smsCalledType, &smsText,	&smsTextEnc);
      //		printf("res=%d\n",res);
      ...
      if (   (res != FB_GSM_UCS2)  &&  (res != FB_GSM_8BIT)	 &&  (res != 0) )
        //FB
      {
        if (res > 0)
      ...
      if (   ((res == FB_GSM_UCS2) || (res == FB_GSM_8BIT)	|| (res == 0))  )
      {
        ...
        sms.tmsi=conn->ue()->tmsi().c_str();		//"007b0001"
        sms.imei= conn->ue()->imei().c_str();
        sms.imsi= conn->ue()->imsi().c_str();		//问题 imsi=MSI值
        sms.connid=conn->ue()->connId();

        //需要检查，是text的pdu,还是bin
        sms.pdu=rpdu->c_str();				//0002000891683108501705f01711a00781053389f90000ff0cb1582c168bc562b1582c06
        if (sms.encode == 0)
        {
          sms.text=smsText.c_str();
          sms.textlen=smsText.length();
        }
        sms.smsc=called.c_str();
        sms.tel=smsCalled.c_str();

        R=SendData_RecvSms(&sms);			//需要判断是自己发出，还是从手机收到的，还是往手机里发的		/iflag? 或tel=0,smsc=0 connid=65535?
      ...
      if (!called)
        Debug(this, DebugNote,
              ...
              m.connId(),
              res ? "empty destination address" : "invalid RP-DATA");
      YBTSSubmit* th = new YBTSSubmit(YBTSTid::Sms, conn->connId(),
                                      conn->ue(), callRef);
      if (called)
      {
      ...
      if (smsText)
      {
        th->msg().addParam("text", smsText);					//文本，原字符串
        th->msg().addParam("text.encoding", smsTextEnc);		//编码方式
        /*
        printf("============smsText====%s\n", smsText.c_str());
        printf("============smsTextEnc====%s\n", smsTextEnc.c_str());
        printf("============rpdu====%s\n", rpdu->c_str());
      ...
      //ShowMsgInfo(__LINE__,th->msg());

      /*
      =========Line=10209==msg(0)  , module  :   ybts
      =========Line=10209==msg(1)  , route_type  :   msg
      =========Line=10209==msg(2)  , caller  :   TMSI007b0001
      =========Line=10209==msg(3)  , tmsi  :   007b0001
      =========Line=10209==msg(4)  , called  :   8613800571500
      =========Line=10209==msg(5)  , callednumplan  :   isdn
      =========Line=10209==msg(6)  , callednumtype  :   international
      =========Line=10209==msg(7)  , sms.called  :   5033989
      =========Line=10209==msg(8)  , sms.called.plan  :   isdn
      =========Line=10209==msg(9)  , sms.called.nature  :   unknown
      =========Line=10209==msg(10)  , text  :   555555111
      =========Line=10209==msg(11)  , text.encoding  :   gsm7bit
      =========Line=10209==msg(12)  , rpdu  :   000e000891683108501705f01411ac0781053389f90000ff09b55aad56abc56231
      =========Line=10209==msg(13)  , phy_info  :   TA=3 TE=0.035 UpRSSI=-5 TxPwr=17 DnRSSIdBm=-48 time=1440510801.839
       */
...
#undef SMS_CPDATA_DONE_MILD
    ...
    if (!cpOk)
    {
      Debug(this, level, "Rejecting SMS CP-DATA conn=%u: %s", conn->connId(),
            reason.c_str());
  ...
  RefPointer<YBTSUE> ue;
  String tmsi(tmsi,strlen(tmsi));

  g_Driver->mm()->getUESafeIdent(ue,tmsi, true, false);
  g_Driver->mm()->
  Message* msg = 0;
  if (! ue)
  return -1;
    ...
    //FB添加完成
        ...
        if (!ue->imsiDetached())
        {
          //Debug(this, DebugInfo, "Detached UE (%p) TMSI=%s IMSI=%s [%p]",	(YBTSUE*) ue, ue->tmsi().c_str(), ue->imsi().c_str(), this);
          //ue->m_imsiDetached = true;
          //ue->se
    ...
    //		ue.
      ...
      {
        msg = g_Driver->mm()->buildUnregister(0, tmsi, true);
    //		g_Driver->mm()->
        Engine::enqueue(msg);
      }
     */
  ...
  int FB_ProcMsg(TSmsInfo * sms)
  {
    ...
    //	printf("fun=%d\n",sms->fun);
    //	if (! g_Driver->signalling())
    //	{
    //		return -1;
    //	}
    //
    //	m_conns=&g_Driver->signalling()->m_conns;
    //
    //
    //	if (! m_conns)
    //	{
    ...
    if (sms->fun == FunDef_TickConn) 		//T
    {
      //		R=FB_findConn(m_conns,sms,0);
      //		if (R == NO_CONN_ID)
      //			return -1;
      //		if (R == -1)
      //			return -1;
      //		g_Driver->signalling()->dropConn(R,true);
      //		//g_Driver->mm()->
      KickConn(sms->tmsi);
      return 1;
    }
    ...
    //RefPointer<YBTSUE> ue;
    snprintf(dest, 128, "TMSI%s\0", sms->tmsi);

    if (sms->fun == FunDef_SendSms)		//
    {
      Message* msg=0;

      //		int R=FB_findConn(m_conns,sms,TMSI);
      //
      //		if ((R = NO_CONN_ID) || (R =-1) || (strlen(TMSI)==0))
      //		{
      //			return -1;
      //		}

      msg=g_Driver->message("msg.execute");
      if (! msg)
      {
        return -1;
      }
      ...
      //	if (! g_Driver->mm()->getUETarget(ue,YSTRING(dest),*msg))
      //	{
      //	//setListParams(msg, s_error, s_reason, "failure");
      //	return -1;
      //	}

      //snprintf(s,10,"%d\0",sms->encode);
      //msg->setParam("encode",s);

      msg->setParam("SMSkey", sms->smsKey);

      if (sms->encode == 4)
      {
        msg->setParam("text.encoding", "gsm8bit");
      }
      if (sms->encode == 8)
      {
        msg->setParam("text.encoding", "UCS2");
      }
      if (sms->encode == 0)
      {
        msg->setParam("text.encoding", "gsm7bit");
      }

      snprintf(s, 100, "%d\0", sms->smscTimeOffset);
      msg->addParam("smscTimeOffset", s);

      if (sms->smsc)
        msg->addParam("caller", sms->smsc);

      if (sms->tel)
      {
        msg->addParam("called", sms->tel);
        msg->addParam("sms.caller", sms->tel);
      }
      if (sms->text)
        msg->addParam("text", sms->text);

      if (sms->textPDU)
        msg->addParam("textPDU", sms->textPDU);
      if (sms->textHex)
        msg->addParam("textHex", sms->textHex);

      if (sms->pdu)
      {
        if (strlen(sms->pdu) > 0)
          msg->addParam("rpdu", sms->pdu);
      }
      if ((! sms->pdu) and  (! sms->text))
      {
        msg->addParam("text", "");
      }
      T=abs(sms->SmsSentOK_KickOut);
      snprintf(s, 100, "%d\0", T);
      msg->addParam("SmsSentOK_KickOut", s);

      T=abs(sms->MaxPPD);
      snprintf(s, 100, "%d\0", T);
      if (T == 0)
        msg->addParam("maxpdd", "5000");			//5秒
      else
        msg->addParam("maxpdd", s);			//5秒

      msg->addParam("handlers", "lateroute:75,jabber:100,sip:100,jingle:100,ybts:100");		//handlers  :   lateroute:75,jabber:100,sip:100,jingle:100,ybts:100


      //b=g_Driver->handleMsgExecute(*msg,YSTRING(TMSI));
      //b=g_Driver->handleMsgExecute(*msg,YSTRING("IMSI\460005....."));
      snprintf(dest, 128, "TMSI%s\0", sms->tmsi);

      String s(dest, strlen(dest));
      b=g_Driver->handleMsgExecute(*msg, s);
    ...
    if (sms->fun == FunDef_BTS_Call)		//
    {
      Message* msg=0;


      msg=g_Driver->message("call.route");
      if (! msg)
      {
        return -1;
      }
      ...
      msg->addParam("caller", sms->caller);
      msg->addParam("called", sms->called);

      //msg->addParam("callednumtype","international");
      msg->addParam("callednumplan", "isdn");
      msg->addParam("username", sms->imsi);
      msg->addParam("imei", sms->imei);
      msg->addParam("emergency", "false");
      msg->addParam("privacy", "true");

      msg->addParam("handlers", "lateroute:75,jabber:100,sip:100,jingle:100,ybts:100");		//handlers  :   lateroute:75,jabber:100,sip:100,jingle:100,ybts:100
      msg->userData(0);

      msg->addParam("oimsi", sms->imsi);
      msg->addParam("otmsi", sms->tmsi);
      ...
      sprintf(s, "%d\0", sms->UpLinkMode);
      msg->addParam("UpLinkMode", s);

      sprintf(s, "%d\0", sms->SmsSentOK_KickOut);
      msg->addParam("SmsSentOK_KickOut", s);
      if (sms->KickOut_RL3Message)
        msg->addParam("KickOut_RL3Message", sms->KickOut_RL3Message);

      String s(dest, strlen(dest));
      //b=g_Driver->msgExecute_2(*msg,s);
  ...
  Message * Get_Message(char * msgtype, TSmsInfo * sms)
  {
    ...
    snprintf(str, 100, "%d\0", sms->smscTimeOffset);
    msg->addParam("smscTimeOffset", str);
    ...
    sprintf(str, "%d\0", sms->UpLinkMode);
    msg->addParam("UpLinkMode", str);

    sprintf(str, "%d\0", sms->SmsSentOK_KickOut);
    msg->addParam("SmsSentOK_KickOut", str);
    if (sms->KickOut_RL3Message)
      msg->addParam("KickOut_RL3Message", sms->KickOut_RL3Message);
    ...
    T=abs(sms->MaxPPD);
    snprintf(str, 100, "%d\0", T);
    if (T == 0)
      msg->addParam("maxpdd", "5000");			//5秒
    else
      msg->addParam("maxpdd", str);			//5秒
  ...
  void * CheckAppMsg_ForceUplink_Loop(void * p)
  {
    int oldstat=0;
    ...
    //printf("p=%p\n",p);
    TSmsInfo sms={0};
    //memmove(&sms,p,sizeof(sms));
    FB_CpyMsg((TSmsInfo *) p, &sms);


    //printf("aCheckAppMsg_ForceUplink_Loop_fun=%d\n",sms.fun);
    //printf("aCheckAppMsg_ForceUplink_Loop_upmode=%d\n",sms.UpLinkMode);
    ...
    if (sms.UpLinkMode == 1)
      msg=Get_Message("call.route", &sms);
    if (sms.UpLinkMode == 2)
      msg=Get_Message("msg.execute", &sms);
    ...
    String s(dest, strlen(dest));
    //printf("CheckAppMsg_ForceUplink_Loop_fun=%d\n",sms.fun);
    //printf("CheckAppMsg_ForceUplink_Loop_upmode=%d\n",sms.UpLinkMode);

    NamedString *nv;
    if (sms.UpLinkMode == 1)
    {
      pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &oldstat);
      while (1)
      {
        pthread_testcancel();
        pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &oldstat);
        g_Driver->msgExecute_Uplink(*msg, s);
        //				nv=msg->getParam("reason");
        //				if (nv)
        //					{
        //					if (strcasecmp(nv->c_str(),"offline")==0)
        //					{
        //						usleep(100000);
        //					}
        //				}
    ...
    if (sms.UpLinkMode == 2)
    {
      ...
      while (1)
      {
        pthread_testcancel();
        pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &oldstat);
        g_Driver->handleMsgExecute(*msg, s);
        //				if (strcasecmp(msg->getParam("reason")->c_str(),"offline")==0)
        //					{
        //						usleep(100000);
        //					}
        //				nv=msg->getParam("reason");
  ...
  void * CheckAppMsg_ForceUplink(void * p)
  {
    int flag=0;
    bool b=false;
    int R=0;
    ...
    while (true)
    {
      ...
      sms=FB_popForceUplink();
      if (sms.fun)
      {
        //printf("bCheckAppMsg_ForceUplink=%d\n",sms.fun);
        flag++;
        //printf("p=%p\n",&sms);
        pthread_create(&CheckAppMsg_ForceUplink_Loop_id, NULL, CheckAppMsg_ForceUplink_Loop, &sms);
        usleep(1000 * 500);
        //      CheckAppMsg_ForceUplink_Loop(&sms);
        //FREE_SmsInfo(&sms);
      }
      if (flag == 0)
        usleep(1000);	//10ms
...
//printf("------------------------aaa---------------\n");
/*
460029888419532=007b0001,353260017691470,8419532,1441651603,ybts/TMSI007b0001
460005815033989=007b0002,353569050600100,5033989,1441651618,ybts/TMSI007b0002			//8X

//
//	=======msg.getParam(0)  , caller  :   8613800571500
//	=======msg.getParam(1)  , called  :   8419532				//��
//	=======msg.getParam(2)  , sms.caller  :   5033989			//��
//	=======msg.getParam(3)  , text  :   msg.text
//	=======msg.getParam(4)  , callto  :   ybts/TMSI007b0001	�
//	=======msg.getParam(5)  , imsi  :   460029888419532		�
//	=======msg.getParam(6)  , tmsi  :   007b0001		�
//	=======msg.getParam(7)  , maxpdd  :   5000
//	=======msg.getParam(8)  , handlers  :   lateroute:75,jabber:100,sip:100,jingle:100,ybts:100
 *
*/
...
/*
		sms.MaxPPD=5000;
		sms.encode=8;
		sms.fun=FunDef_SendSms;
		sms.imsi="353569050600100";
		sms.imsi="460005815033989";
		sms.smsc="8613500101234";
		//sms.tmsi="007b0002";
		sms.tel="123456798";
		sms.textPDU="054E2D4E2D0061";
		 // sms.pdu="01000891683105101032F4001C04098121436597F80008518003513521000A4E2D006100614E2D0061";
        sms.pdu="";
		sms.pdu="01000891683105101032F4001904098121436597F80008518003414523000C006100620063006400650066";
*/
...
//			printf("		--------------------------sssss-------------\n");
  ...
  void * CheckAppMsg_OtherTask(void * p)
  {
    int flag=0;
    bool b=false;
    int R=0;
    ...
    while (true)
    {
        ...
        blnofree=0;
        //printf("CheckAppMsg_OtherTask=%d\n",sms.fun);
        flag++;
        //printf("p=%p\n",&sms);
        if (sms.fun == FunDef_askIMEI)
        {
          if (sms.askIMEI == 1)
            s_askIMEI=true;
          else
        ...
        if (sms.fun == FunDef_NewLocation)
        {
          YBTSSignalling* sig=g_Driver->signalling();
          
          
          //2016-05-03_07:05:54.773955 <ybts:INFO> LAI changed (null) -> 46001_03e8
          
          const YBTSLAI & lai=sig->lai();
          YBTSLAI lai2;
...
//          sig->lai()=lai2;
          s_lai=lai2;
          g_Driver->signalling()->NewLAI(s1,s2,s3);
          //printf("       lai=%s\n",s_lai.lai().c_str());          
          
          //__plugin.signalling()->lai().m_lai=lai2.lai();
          //__plugin.signalling()->lai().set(s1, s2, s3);
  ...
  void YBTSDriver::start()
  {
    ...
    while (true)
    {
      ...
      g_Driver=this;
      //printf("--------------------startserver--\n");
      if (FB_ServerThread_id == -1)
      {
        SendData_Log(-1, "====启动低层通讯服务====");
        StartServer();
        //FB添加完成
        if   (CheckAppMsg_SMS_id == -1)
          pthread_create(&CheckAppMsg_SMS_id, NULL, CheckAppMsg, NULL);
        if (CheckAppMsg_Call_id == -1)
          pthread_create(&CheckAppMsg_Call_id, NULL, CheckAppMsg_Call, NULL);
        if (CheckAppMsg_ForceUplink_id == -1)
          pthread_create(&CheckAppMsg_ForceUplink_id, NULL, CheckAppMsg_ForceUplink, NULL);
        if (CheckAppMsg_OtherTask_Loop_id == -1)
          pthread_create(&CheckAppMsg_OtherTask_Loop_id, NULL, CheckAppMsg_OtherTask, NULL);
  ...
  void SendSMSStatus(Message& msg, const String& dest, int ok, char * Reason)
  {
    const char * SrcTel=0;
    const char * DestTmsi=0;
    const char * smskey=0;

    String s1;
    String s2;

    s1=msg.getParam("SMSkey");
    if (s1)
    {
      smskey=s1.c_str();
      if (smskey == 0)
    ...
    s2=msg.getParam("called");
    if (s2)
    {
      SrcTel=s2.c_str();
    }

    FB_SendSms_Status(SrcTel, DestTmsi, smskey, ok, Reason);
  }

  static const char* s_mmMsgs2[] ={
    ...
    // Security messages
    //"AuthenticationReject",     //虽然可以T出网络,再也不入网了,手机显示SIM注册失败
  ...
  bool YBTSDriver::handleMsgExecute(Message& msg, const String& dest)
  {
    ...
    while (TelEngine::null(rpdu))
    {
      ...
      /*
          ...
          {
            Debug(this, DebugNote, "MT SMS: unknown encoding '%s'",
                enc.c_str());
            break;
          }
          if (!text)
      ...
      unsigned int smscTimeOffset = msg.getIntValue(YSTRING("smscTimeOffset"), 0);
      //	printf("GSMTimeOffset=		%d\n",smscTimeOffset);
      smscTs=smscTs + smscTimeOffset;
      ...
      //FB添加
      int encode=0;
      //const char * encodestr=msg.getParam("text.encoding")->c_str();
      encode=msg.getIntValue(YSTRING("encode"));
      //FB_printbuf(tpdu.data(),tpdu.length(),"tpdu");
      if (enc && enc.toLower() == YSTRING("gsm7bit"))
      {
      ...
      //FB_printbuf(tpdu.data(),tpdu.length(),"tpdu");
      const NamedString * textPDU = msg.getParam("textPDU");
      if  (textPDU)
      {
        unsigned char X[1024]={0};
        len = HexStr2Bin(textPDU->c_str(), &X[0]);
        sms.assign((void * ) &X[0], len);
        //FB_printbuf(sms.data(),sms.length(),"sms");
...
#define YBTS_SMSOUT_DONE(reason) { \
                    sms->terminate(false,reason);\
                    SendSMSStatus(msg,dest,0,reason); \
                    break; }\
//FB修改完成
...
#undef YBTS_SMSOUT_DONE
    ...
    //FB添加
    if  (ok)
      SendSMSStatus(msg, dest, 1, 0);
    int SmsSentOK_KickOut=msg.getIntValue(YSTRING("SmsSentOK_KickOut"));
    /*
    if (SmsSentOK_KickOut==1)
    {
        //UpdateLAC();
        ...
        char Str[20]={0};
        sprintf(Str,"%d",T);
        //this->signalling()->lai().set("460","00",Str);
        /*
        YBTSLAI lai;
        ...
        if (mcc.length() == 3 && isDigits09(mcc) &&   ((mnc.length() == 3 || mnc.length() == 2) && isDigits09(mnc))) 
        {
        ...
        this->signalling()->lai().reset();
        this->signalling()->lai().operator =(lai);
        this->signalling()->lai().build().
     */

    //printf("ueconid=  %d",ue->connId());
    ...
    /*
    const NamedString * RL3_Str = msg.getParam("KickOut_RL3Message");        
    char RL3_Str2[512]={0};
    if (RL3_Str)
      {
        if (strlen(RL3_Str->c_str ())>0)
          {
            sprintf(RL3_Str2,"%s",RL3_Str->c_str ());
          }
      }
     //
    ...
    int c=this->signalling()->m_conns.count();        
    char str1[256]={0};
    char str2[256]={0};
    char str3[256]={0};
    ...
    for (int i = c-1; i>=0; i--) 
      {
        conn=(YBTSConn *)this->signalling()->m_conns.at(i);
        printf("                          connID           %d ,  %d\n",conn->connId(),conn->m_flags);
            
            
        //this->mm()->sendIdentityRequest(conn,1);
        //m=new YBTSMessage;
        //m->setConnId(conn->connId()) ;            
        //this->mm()->handleIdentityResponse(*m,0,conn);
            
        //this->signalling()->authReject(conn);
        ...
        if (msg.getParam ("imei"))
          strcpy(str1,msg.getParam ("imei")->safe ());
        if (msg.getParam ("imsi"))
          strcpy(str2,msg.getParam ("imsi")->safe ());
        if (msg.getParam ("tmsi"))
          strcpy(str1,msg.getParam ("tmsi")->safe ());
            
        //this->mm ()->sendIdentityRequest(conn,YBTSConn::FAskIMSI);
        //KickOut(ue, conn, str1,str2,str3);
                                                
            //conn->sendL3(YBTSMM::buildMM("AuthenticationReject"));       //铁定掉线，而且sim卡注册失败，需要手工选择网络才能再次入网

                
            //要只 T connid对上的，其它的不要，可能其它的刚入网还没发短信
                
                
        }
    //this->signalling()->m_conns->unlock ();
}
     */
    //FB完成
  ...
  bool YBTSDriver::msgExecute_Uplink(Message& msg, String& dest)
  {
    ...
    //for (i=0;i<100;i++ )
    {
      ue->startPaging(ChanTypeVoice);

      usleep(10000); //FB 2016-4-26
      ue->stopPagingNow();
      usleep(50);
      ue->startPaging(ChanTypeSMS);
      //ue->startPaging(ChanTypeVoice);
      //ue->startPaging(ChanTypeSS);
      
      usleep(10000);   //FB 2016-4-26
      ue->stopPagingNow();
      usleep(50);   //FB 2016-4-26
  ...
  bool YBTSDriver::msgExecute(Message& msg, String& dest)
  {
    ...
    bool ok = chan->initOutgoing(msg);		//内部可能跳转区别
    lckCallStart.drop();
    chan->deref();
    if (ok)
    {
