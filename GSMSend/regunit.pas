...
type                //一起写入， 总大小不超4K
  Tmmcblk0rpmb_Head=packed record   //sizeof=128
...
//    Product:integer;
  ...
  Tmmcblk0rpmb_DataItem=packed record   //sizeof=64
  ...
  Tmmcblk0rpmb_Data=packed record         //256
    SDRSN:Tmmcblk0rpmb_DataItem;
    SysUUID:Tmmcblk0rpmb_DataItem;
    SysSN:Tmmcblk0rpmb_DataItem;
    SysBrdSN:Tmmcblk0rpmb_DataItem;
    NetCard:Tmmcblk0rpmb_DataItem;

    Count:Tmmcblk0rpmb_DataItem;
    AllCount:Tmmcblk0rpmb_DataItem;
    //PayCount:Tmmcblk0rpmb_DataItem;

    DiskInfo:Tmmcblk0rpmb_DataItem;
    DogInfo:Tmmcblk0rpmb_DataItem;
    Res:Tmmcblk0rpmb_DataItem;
    //后面的，Name必须不为空，才是有效的，否则不检查CRC
  end;

  Pmmcblk0rpmb=^Tmmcblk0rpmb;
  Tmmcblk0rpmb=packed record
    Head:Tmmcblk0rpmb_Head;
    Data:Tmmcblk0rpmb_Data;
  end;
...
procedure mmcblk0rpmb_SetData(var D:Tmmcblk0rpmb_DataItem;Name:ansiString;Data:AnsiString);

procedure mmcblk0rpmb_EncodeItem(var D:Tmmcblk0rpmb_DataItem);
function mmcblk0rpmb_DecodeItem(var D:Tmmcblk0rpmb_DataItem):Boolean;

function GetMacID():String;
function Str2MacID(str:String;out M:TMacID):Boolean;
function CheckCountSaveAddr():Boolean;
...
uses
  fununit,CRC16_Unit,CRC8_Unit,dmunit,commfununit;


const                          //改成一个列表，用于查找可用的mmc
  mmcblk0rpmb_File_Str='17405D54174D535A54654B03595B5B5B5E530872484156';                  //lib/tables/mmcblk0rpmb
...
{
if sysutils.FileExists(mmcblk0rpmb_File) then
mmcblk0rpmb:=true
else
mmcblk0rpmb:=false;
}
...
begin
  ...
  Listnetcard(strlst,false,true,true,false,2);
  ...
  begin
    ...
    mmcblk0rpmb_SetData(R.Data.NetCard,'NetCard',s);
...
begin
  ...
  ListNetCard(strlst,true,true,true,false,2);
...
//      exit;
  ...
  strlst2.DelimitedText:=R.Data.NetCard.Data;
  c:=0;
  if strlst2.Count>0 then
  begin
    for i := 0 to strlst2.Count-1 do
    begin
      if strlst.IndexOf(strlst2.ValueFromIndex[i])<>-1 then
      inc(c);
    end;
    if (c<2) then
    begin
...
//  fn:=mmcblk1rpmb_File;
  ...
  mmcblk0rpmb_EncodeItem(R.Data.NetCard);
...
//  exit;
  if not mmcblk0rpmb_DecodeItem(R.Data.NetCard) then
  exit;
...
//    exit;
  ...
  if pos('VMware-56 4d 8d 77',s)=1 then
  begin
    Result:=true;
//    mmcblk0rpmb:=False;
  end;

  if pos('LXPXT01005037',s)=1 then
  begin
    Result:=true;
//    mmcblk0rpmb:=false;
  end;
