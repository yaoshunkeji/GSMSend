...
const
  // Digest for empty string: Md5Null := MD5String('');
  Md5Null: TMD5Digest =
    (A: 3649838548; B: 78774415; C: 2550759657; D: 2118318316);
...
// Compare two hashsum
function MD5Compare(const D1, D2: TMD5Digest): Integer;
...
type
  PArray4LongWord = ^TArray4LongWord;
...
procedure Transform(State: PArray4LongWord; Buffer: Pointer);
var
  A, B, C, D: LongWord;
...
begin
  Move(Context.Count, Bits, SizeOf(TArray2LongWord));
  Index := Context.Count[0] shr 3 and $3F;
  if Index < 56 then
    PadLen := 56 - Index
  else
  ...
  Move(Context.State, Digest, SizeOf(TArray4LongWord));
end;
...
function MD5Compare(const D1, D2: TMD5Digest): Integer;
  ...
  function LongWordCompare(const N1, N2: LongWord): Integer;
  begin
    Result := 0;
    if N1 = N2 then
      Exit;
...
begin
  Result := 0;
  if MD5DigestEquals(D1, D2) then
    Exit;
  Result := LongWordCompare(D1.A, D2.A);
  if Result <> 0 then
    Exit;
  Result := LongWordCompare(D1.B, D2.B);
  if Result <> 0 then
    Exit;
  Result := LongWordCompare(D1.C, D2.C);
  if Result <> 0 then
    Exit;
  Result := LongWordCompare(D1.D, D2.D);
end;
